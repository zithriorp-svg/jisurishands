...
export async function GET() {
  try {
    ...
    // 🚀 NEW: CASH FLOW VELOCITY (90-DAY MATRIX / WEEKLY)
    ...
    ledgers.forEach(l => {
      ...
      if (lDate >= ninetyDaysAgo) {
        ...
        if (l.debitAccount === "Vault Cash") entry.capitalIn += Number(l.amount); // Money coming in
        if (l.creditAccount === "Vault Cash") entry.capitalOut += Number(l.amount); // Money going out
      }
    });
    ...
    // 🚀 We now pass cashFlowVelocityData into the response payload!
